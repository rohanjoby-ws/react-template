...
describe('Tests for getCurrentRouteDetails method', () => {
  ...
  it('should return the details of the route', () => {
    let locationHome = { pathname: '/' };
    expect(getCurrentRouteDetails(locationHome)).toEqual(routeConstants['iTunes']);
    let locationDetails = { pathname: '/track/:trackId' };
    expect(getCurrentRouteDetails(locationDetails)).toEqual(routeConstants['iTunesDetails']);
  });
